...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fade_trader.log'),
...
class FadeEngine:
    ...
    def update_price(self, symbol: str, price: float, timestamp: float = None) -> Optional[FadeSignal]:
        ...
        if abs_move >= self.min_move_threshold:
            # ABOVE THRESHOLD: Use original expansion logic (same as before)
            excess_move = abs_move - self.min_move_threshold
            ...
            # Only increase position size (ratchet up), never contract above threshold
            ...
            # BELOW THRESHOLD: Position-aware scaling logic
            if current_position == 0:
                # No position and below threshold - stay flat
                return None

            # We have a position - apply position-aware scaling
            peak_position = self.peak_positions[symbol]
            excess_move = 0.0  # Below threshold, no excess move

            if current_position > 0:  # We're long
                # Scale down as price moves UP (favorable direction)
                distance_from_high = window_high - current_price
                scale_factor = max(0, distance_from_high / self.min_move_threshold)
                goal_position = int(peak_position * scale_factor)
            else:  # We're short
                # Scale down as price moves DOWN (favorable direction)
                distance_from_low = current_price - window_low
                scale_factor = max(0, distance_from_low / self.min_move_threshold)
                goal_position = int(peak_position * scale_factor)

            # Only contract below threshold, never expand
